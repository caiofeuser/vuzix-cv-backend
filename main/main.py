...
try:
    ...
    logging.error(f"Error configuring the Gemini API. Check your API Key. Error: {e}")
...
try:
    ...
    logging.error(f"Error loading generic_model.tflite or generic_labels.txt: {e}")
    exit()
    ...
    try:
        while True:
            ...
            if msg_type == "detection":
                detection_results = run_object_detection(image, input_width, input_height, interpreter, output_details, input_details)
                response = {
